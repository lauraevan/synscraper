...
export const Navbar = () => {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled || mobileOpen || gearOpen ? "border-b border-[#ffd400]/10 bg-[#080807]/94 backdrop-blur-xl" : "bg-gradient-to-b from-black/80 via-black/35 to-transparent"}`}>
      <div className="relative mx-auto flex h-[68px] max-w-[1500px] items-center px-5 md:px-8">
        ...
        <div className="absolute left-1/2 hidden -translate-x-1/2 items-center md:flex">
          <nav className="flex items-center gap-1 rounded-full border border-[#ffd400]/10 bg-black/30 p-1">
            {NAV_ITEMS.map((item) => (
            ...
            <div ref={gearRef} className="relative">
              <button
                ...
                className={`grid h-8 w-8 place-items-center rounded-full transition ${gearOpen ? "bg-[#ffd400] text-black" : "text-white/52 hover:bg-[#ffd400]/10 hover:text-[#ffd400]"}`}
                aria-label="Open SynFlix menu"
                aria-expanded={gearOpen}
              >
                <Settings className={`h-4 w-4 transition-transform duration-300 ${gearOpen ? "rotate-45" : ""}`} />
              </button>

              {gearOpen && (
                <div className="absolute right-0 top-[43px] w-[300px] overflow-hidden rounded-[20px] border border-white/[0.08] bg-[#0b0b0a] p-2 shadow-[0_24px_70px_rgba(0,0,0,.62)]">
                  ...
                  {UTILITY_ITEMS.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl border ${selected ? "border-[#ffd400]/26 bg-[#ffd400]/10 text-[#ffd400]" : "border-white/[0.07] bg-[#11110f] text-white/48"}`}><Icon className="h-4 w-4" /></span>
      ...
      {mobileOpen && (
        <div className="border-t border-[#ffd400]/10 bg-[#080807] px-5 pb-5 pt-4 md:hidden">
          <form onSubmit={submit} className="mb-3 flex h-11 items-center gap-2 rounded-2xl border border-white/[0.09] bg-[#10100f] px-3 focus-within:border-[#ffd400]/35">
          ...
          <nav className="grid grid-cols-3 gap-2">
            {NAV_ITEMS.map((item) => {
              const Icon = item.icon;
              return (
                <Link key={item.to} to={item.to} className={`flex flex-col items-center justify-center gap-2 rounded-2xl border px-2 py-3.5 text-xs font-medium transition ${active(item) ? "border-[#ffd400] bg-[#ffd400] text-black" : "border-white/[0.08] bg-[#10100f] text-white/68"}`}>
          ...
          <div className="mt-3 rounded-2xl border border-white/[0.07] bg-[#0d0d0c] p-2">
