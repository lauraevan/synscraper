...
const CinematicCard = ({ item }) => {
  ...
  return (
    <article
      ...
      className="group relative w-[78vw] max-w-[520px] shrink-0 snap-start cursor-pointer overflow-hidden rounded-[14px] border border-white/[0.08] bg-[#10100f] shadow-[0_18px_50px_rgba(0,0,0,.24)] transition duration-500 hover:-translate-y-1 hover:border-white/20 hover:shadow-[0_24px_64px_rgba(0,0,0,.44)] sm:w-[440px] lg:w-[500px]"
    >
      <div className="relative aspect-[16/9] overflow-hidden">
        {art ? (
          <img src={art} alt={titleOf(item)} loading="lazy" decoding="async" className="h-full w-full object-cover transition duration-700 ease-out group-hover:scale-[1.035]" />
        ) : (
        ...
        <div className="absolute inset-x-0 bottom-0 p-4 sm:p-5">
          <div className="flex items-end justify-between gap-4">
            ...
            <div className="flex shrink-0 translate-y-2 items-center gap-2 opacity-0 transition duration-300 group-hover:translate-y-0 group-hover:opacity-100">
              <button onClick={play} className="grid h-10 w-10 place-items-center rounded-full bg-[var(--site-accent,#ffd400)] text-black shadow-lg transition hover:scale-105" aria-label="Play"><Play className="ml-0.5 h-4 w-4 fill-current" /></button>
              <button onClick={save} className="grid h-10 w-10 place-items-center rounded-full border border-white/18 bg-black/50 text-white backdrop-blur-md transition hover:border-white/35 hover:bg-white/10" aria-label={saved ? "Remove from My List" : "Add to My List"}>{saved ? <Check className="h-4 w-4" /> : <Plus className="h-4 w-4" />}</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <section className="group/cinematic py-7 md:py-9" data-testid={testId}>
      <div className={`mb-4 flex items-end justify-between gap-4 px-5 md:px-8 ${reverse ? "md:flex-row-reverse md:text-right" : ""}`}>
        ...
        <div className="hidden items-center gap-1.5 md:flex">
          <button onClick={() => scroll(-1)} className="grid h-8 w-8 place-items-center rounded-lg border border-white/[0.08] bg-white/[0.035] text-white/48 transition hover:bg-white/[0.08] hover:text-white" aria-label="Scroll left"><ChevronLeft className="h-4 w-4" /></button>
          <button onClick={() => scroll(1)} className="grid h-8 w-8 place-items-center rounded-lg border border-white/[0.08] bg-white/[0.035] text-white/48 transition hover:bg-white/[0.08] hover:text-white" aria-label="Scroll right"><ChevronRight className="h-4 w-4" /></button>
        </div>
      </div>
      <div
        ref={ref}
