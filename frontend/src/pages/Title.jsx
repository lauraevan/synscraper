  ...
  return (
    <section className="mx-auto max-w-[1500px] px-5 py-8 md:px-8" data-testid="season-picker">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <select data-testid="season-select" value={sel} onChange={(e) => setSel(Number(e.target.value))} className="rounded-full border border-[#ffd400]/18 bg-white/[0.04] px-4 py-2 text-xs font-medium text-white outline-none focus:border-[#ffd400]/45">
          {valid.map((s) => <option key={s.id} value={s.season_number} className="bg-[#111]">{s.name}</option>)}
        </select>
      </div>
      <div className="grid gap-2.5">
        {(data?.episodes || []).map((ep) => (
          <button key={ep.id} data-testid={`episode-${ep.episode_number}`} onClick={() => navigate(`/watch/tv/${id}?season=${sel}&episode=${ep.episode_number}`)} className="group flex gap-4 rounded-2xl border border-white/[0.07] bg-white/[0.025] p-3 text-left transition hover:border-[#ffd400]/22 hover:bg-[#ffd400]/[0.035]">
            <div className="relative aspect-video w-32 shrink-0 overflow-hidden rounded-xl bg-black md:w-44">
              ...
              <div className="absolute inset-0 grid place-items-center bg-black/35 opacity-0 transition group-hover:opacity-100"><span className="grid h-9 w-9 place-items-center rounded-full bg-[#ffd400] text-black"><Play className="h-4 w-4 fill-current" /></span></div>
            </div>
  ...
  return (
    <main className="min-h-screen bg-[#070706]" data-testid="title-page">
      <section className="relative h-[72vh] min-h-[560px] max-h-[840px] overflow-hidden">
        ...
        <button onClick={() => navigate(-1)} className="absolute left-5 top-24 z-10 grid h-10 w-10 place-items-center rounded-full border border-white/12 bg-black/38 text-white/72 backdrop-blur-md transition hover:border-[#ffd400]/35 hover:bg-[#ffd400]/10 hover:text-[#ffd400] md:left-8"><ChevronLeft className="h-5 w-5" /></button>
      ...
      {cast.length > 0 && (
        <section className="mx-auto max-w-[1500px] px-5 py-8 md:px-8">
          <div className="mb-1 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#ffd400]/60"><span className="h-px w-5 bg-[#ffd400]/55" /> People</div>
